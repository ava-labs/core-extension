...
Translation files live in `apps/next/src/localization/locales/<locale>/translation.json`. English (`en`) is the source language and is generated by the scanner; every other locale is written by Crowdin.

`crowdin.yml` maps the source (`locales/en/**/*.json`) onto `locales/%locale%/%original_file_name%`, so a new locale needs no code change beyond being enabled in Crowdin.

The scanner is configured in `apps/next/i18next-scanner.config.js`. It scans `apps/next/src`, `packages/ui/src`, and `packages/common/src` — texts added anywhere else will not be picked up.
