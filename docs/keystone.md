...
The document about how to implement keystone to the extension. The basic approach is to generate and read QR codes with the Keystone device.
The current firmware version is M-8.7

## What we have discovered so far

There is an App for the Aclanche (it reuires a beta firmware for the time being). Go to the Menu -> Software Waller -> Core Wallet and select that by tap on the Confirm on the top right corner.

### Create Wallet

For create a new wallet we need a QR code from keystone.
There are three dots on the top right corner, tap on those and choose the Connect Software Wallet option. It's diplaying a QR code not e.g.: `UR:CRYPTO-MULTI-ACCOUNTS/SOMELONGSTRING`
Wee need to decode that given code and get addresses from that:
...
```
 // EXAMPLE TRANSACTON PARAMS FOR EVM
...
For this we will need a bit tweek our existing psbt to add a property called `bip32Derivation`.
...
After that the QR code generating:
...
But we should use for transaction only two: "eth-sign-request" form C-Chain tx and "crypto-psbt" for BTC.
