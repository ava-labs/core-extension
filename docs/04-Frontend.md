...
Even though we only have a single bundle, the frontend has multiple entry points created during the build process. Use `isSpecificContextContainer` from `@core/ui` (`packages/ui/src/utils/isSpecificContextContainer.ts`) to determine the current context the frontend is running in.
