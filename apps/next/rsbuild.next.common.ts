...
export interface CommonConfigOptions {
   ...
   * for shippable builds (alpha/prod) so CI can flag policy drift; disabled
   * for local dev so editing app code can't silently widen permissions.
   */
      ...
      rspack: (_, { appendPlugins, appendRules }) => {
        ...
        appendPlugins(
          new LavaMoatRspackPlugin({
            // Driven per build target: alpha/prod regenerate so PR CI can fail
            // on policy drift, dev loads the checked-in policy so local edits
