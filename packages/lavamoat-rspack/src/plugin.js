...
const {
  wrapGenerator,
  wrapModuleCodegenSource,
} = require('./buildtime/generator.js');
...
const PLUGIN_NAME = 'LavaMoatRspackPlugin';

/**
 * Rspack runs JS module codegen in Rust; LavaMoat hooks NMF generators for webpack
 * but those never run. Rsdoctor's builtin drives `moduleSources` after codegen so we
 * can patch sources. The builtin must be present; we add a minimal config if missing.
 *
 * @param {import('@rspack/core').Compiler} compiler
 */
function ensureRsdoctorForLavaMoat(compiler) {
  const { plugins } = compiler.options;
  const hasRsdoctorBuiltin = plugins.some(
    (p) =>
      !!p &&
      typeof p === 'object' &&
      /** @type {{ constructor?: { name?: string } }} */ (p).constructor
        ?.name === 'RsdoctorPlugin',
  );
  // Mutating `plugins` here is too late: createCompiler has already called `.apply`
  //_on the original list. Register the native Rsdoctor builtin immediately.
  if (!hasRsdoctorBuiltin) {
    new RsdoctorPlugin({
      moduleGraphFeatures: ['graph', 'ids', 'sources'],
      chunkGraphFeatures: [],
    }).apply(compiler);
  }
}
...
class LavaMoatRspackPlugin {
  ...
  constructor(options = {}) {
    ...
    this.options = {
      policyLocation: path.join('lavamoat', 'rspack'),
  ...
  apply(compiler) {
    ...
    ensureRsdoctorForLavaMoat(compiler);
     ...
     * @property {Map<number, string>} [rsdoctorUkeyToIdentifier]
    ...
    // loadCanonicalNameMap uses browser-resolve; the bundler's async resolver is not wired here.
    ...
    compiler.hooks.thisCompilation.tap(
      ...
      (compilation, { normalModuleFactory }) => {
        ...
        // Validation runs in afterSeal so it follows runtime hooks and assets.
        ...
        // =================================================================
        // Path/policy pass: run after module ids (Rsdoctor moduleIds hook). Rspack assigns
        // chunkGraph module ids before codegen; chunkHash is too late for moduleSources wrapping.
        /**
         * @param {Iterable<import('@rspack/core').Chunk>} chunks
         * @param {Iterable<import('@rspack/core').Module>} modules
         */
        const runPathsAndPolicyPass = (chunks, modules) => {
          assertFields(STORE, [
            'options',
          ...
          STORE.pathToResourceId = identifierLookup.pathToResourceId;
          ...
          STORE.runtimeOptimizedPolicy = identifierLookup.getTranslatedPolicy();
        ...
        /** @type {boolean} */
        let ranPathsPass = false;

        const rsdoctorHooks = RsdoctorPlugin.getCompilationHooks(compilation);

        rsdoctorHooks.moduleIds.tapPromise(PLUGIN_NAME, async (patch) => {
          if (ranPathsPass) {
            return;
          }
          /** @type {Map<number, string>} */
          const ukeyToIdentifier = new Map();
          const chunkGraph = compilation.chunkGraph;
          /** @type {Map<string, string>} */
          const renderIdToIdentifier = new Map();
          for (const mod of compilation.modules) {
            const mid = chunkGraph.getModuleId(mod);
            if (mid != null) {
              renderIdToIdentifier.set(String(mid), mod.identifier());
            }
          }
          for (const row of patch.moduleIds) {
            const identifier = renderIdToIdentifier.get(String(row.renderId));
            if (identifier !== undefined) {
              ukeyToIdentifier.set(row.module, identifier);
            }
          }
          STORE.rsdoctorUkeyToIdentifier = ukeyToIdentifier;

          ranPathsPass = true;
          try {
            runPathsAndPolicyPass(compilation.chunks, compilation.modules);
        ...
        rsdoctorHooks.moduleSources.tapPromise(PLUGIN_NAME, async (patch) => {
          if (PROGRESS.isCancelled() || !STORE.pathToResourceId) {
            return;
          }
          const ukeyToIdentifier = STORE.rsdoctorUkeyToIdentifier;
          if (!ukeyToIdentifier) {
            compilation.errors.push(
              new WebpackError(
                'LavaMoatPlugin: Rsdoctor module graph missing before moduleSources.',
              ),
            );
            return;
          }

          /** @type {Map<string, import('@rspack/core').Module>} */
          const idToModule = new Map();
          for (const m of compilation.modules) {
            idToModule.set(m.identifier(), m);
          }

          for (const row of patch.moduleOriginalSources) {
            const identifier = ukeyToIdentifier.get(row.module);
            if (identifier === undefined) {
              continue;
            }
            const mod = idToModule.get(identifier);
            if (
              !mod ||
              !COVERED_MODULE_TYPES.includes(
                /** @type {string} */ (mod.type),
              ) ||
              Object.getPrototypeOf(mod).constructor.name !== 'NormalModule'
            ) {
              continue;
            }
            try {
              const next = wrapModuleCodegenSource({
                module: /** @type {import('@rspack/core').NormalModule} */ (
                  mod
                ),
                source: row.source,
                getIdentifierForPath: STORE.pathToResourceId,
                excludes: STORE.excludes,
                PROGRESS,
              });
              row.source = next;
              row.size = Buffer.byteLength(next, 'utf8');
            } catch (/** @type {any} */ err) {
        ...
        // =================================================================
        // END OF path/policy + Rsdoctor moduleSources wrapping

        // =================================================================
        // Add LavaMoat runtime via compilation runtime requirements hooks.
        ...
        compilation.hooks.additionalTreeRuntimeRequirements.tap(
          ...
          (chunk /*, set*/) => {
            if (chunk.hasRuntime()) {
              // Under Rspack, wrapping happens in Rsdoctor moduleSources (after codegen), which runs
              // after the moduleIds path/policy pass. Runtime chunks may be finalized earlier, so we
              // key off pathsProcessed instead of generatorCalled.
              if (!PROGRESS.done('pathsProcessed')) {
                ...
                diag.rawDebug(
                  2,
                  'skipped adding runtime (additionalTreeRuntimeRequirements)',
                );
              } else {
        ...
        if (STORE.options.inlineLockdown) {
          ...
          const htmlTemplatePlugin = compiler.options.plugins.find(
            /**
             * @param {unknown} plugin
             * @returns {plugin is import('@rspack/core').WebpackPluginInstance}
             */
            (plugin) =>
              !!plugin &&
          ...
          compilation.hooks.processAssets.tap(
            ...
            sesEmitHook({
              ...
              htmlTemplatePluginInterop:
                !!STORE.options.htmlTemplatePluginInterop,
            }),
          );
        }
...
module.exports = LavaMoatRspackPlugin;

module.exports.LavaMoatRspackPlugin = LavaMoatRspackPlugin;
module.exports.LavaMoatPlugin = LavaMoatRspackPlugin;
