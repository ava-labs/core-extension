...
import { lockThemeToDark } from '../helpers/extensionHelpers';

test.describe('Onboarding', () => {
  test('As a CORE ext user, I can see Google, Apple, Manually create wallet, and Access existing wallet options', async ({
    extensionPage,
  }, testInfo) => {
  ...
  test('As a CORE ext user, on the onboarding page, I can check the language dropdown box and verify all languages are selectable', async ({
    extensionPage,
  }, testInfo) => {
    testInfo.annotations.push({
      type: 'testrail_case_field',
      description: 'custom_automation_id:EXT_ONBOARDING_002',
    });
  ...
  test('As a CORE ext user, when I select the Access existing wallet option, I can see Recovery Phrase, Ledger and Keystone options', async ({
    extensionPage,
  }, testInfo) => {
    testInfo.annotations.push({
      type: 'testrail_case_field',
  ...
  test('As a CORE ext user, for the Access Recovery Phrase option, 12-24 words can be selectable, Clear All and Next buttons can be functional', async ({
    extensionPage,
  }, testInfo) => {
    testInfo.annotations.push({
      type: 'testrail_case_field',
      description: 'custom_automation_id:EXT_ONBOARDING_004',
    });
  ...
  test('As a CORE ext user, for the Access Recovery Phrase option, an Invalid Phrase error can be displayed if the user types the wrong one', async ({
    extensionPage,
  }, testInfo) => {
    testInfo.annotations.push({
      type: 'testrail_case_field',
      description: 'custom_automation_id:EXT_ONBOARDING_005',
    });
  ...
  test(
    ...
    async ({ extensionPage, context }, testInfo) => {
      ...
      extensionPage.on('request', (request) => {
        const url = request.url();
        if (url.includes('locales') || url.includes('core.app')) {
