...
import { DappPermissions, PermissionEvents, Permissions, PERMISSION_STORAGE_KEY } from './models';
...
describe('background/services/permissions/PermissionsService.ts', () => {
  ...
  describe('getPermissions', () => {
    it('returns permissions from storage', async () => {
      ...
      (storageService.load as jest.Mock).mockResolvedValue({...mockPermissionData})
    ...
    it('returns empty object in case of load error', async () => {
      ...
      (storageService.load as jest.Mock).mockRejectedValue(new Error('storage error'))
    ...
    it('emits new values on first read', async () => {
      ...
      (storageService.load as jest.Mock).mockResolvedValue({...mockPermissionData})
      permissionService.addListener(PermissionEvents.PERMISSIONS_STATE_UPDATE, eventListener)
  ...
  describe('getPermissionsForDomain', () => {
    it('returns permissions for a given domain', async () => {
      ...
      const result = await permissionService.getPermissionsForDomain('oneaccount.example');
  ...
  describe('addPermission', () => {
    it('adds new permission for new domain and saves it to storage', async () => {
      ...
      const newPermission = {...mockPermissionData['oneaccount.example']} as DappPermissions
    ...
    it('updates already existing permissions', async () => {
      ...
      const newPermission = {domain: 'oneaccount.example', accounts: {
  ...
  describe('setAccountPermissionForDomain', () => {
    it('changes permission of an already added account', async () => {
      ...
      expect(permissions['oneaccount.example']?.accounts['0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee']).toBe(true);

      await permissionService.setAccountPermissionForDomain('oneaccount.example', '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', false);

      const newPermissions = await permissionService.getPermissions();
      expect(newPermissions['oneaccount.example']?.accounts['0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee']).toBe(false);
    });
    ...
    it('adds domain if missing', async () => {
      ...
      const permissions = await permissionService.getPermissions();
      expect(permissions['newdomain.example']?.accounts['0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee']).toBe(undefined);

      await permissionService.setAccountPermissionForDomain('newdomain.example', '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', true);

      const newPermissions = await permissionService.getPermissions();
      expect(newPermissions['newdomain.example']?.accounts['0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee']).toBe(true);
    });
