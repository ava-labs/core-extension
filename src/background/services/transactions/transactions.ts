...
import { tap, filter, map, mergeWith } from 'rxjs/operators';
import { firstValueFrom, Observable, Subject } from 'rxjs';
class TransactionsService extends IndexAndStoreBase<Transaction> {
  addTransaction(data: JsonRpcRequest<txParams[]>) {
    ...
    return {
      listenForTxPending: (cancel: Observable<any>) => {
  ...
  listenForTransactionFinalized(id: string, cancled?: Observable<any>) {
    return firstValueFrom(
      storageListener.pipe(
        map(() => this.getById(id)?.txHash),
        filter((result) => !!result),
        mergeWith(cancled || new Subject())
      )
    );
  }
   ...
   */
  listenForTransactionPending(id: string, cancled: Observable<any>) {
    return firstValueFrom(
      storageListener.pipe(
